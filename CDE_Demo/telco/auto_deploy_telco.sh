...
cde resource upload-archive --name countries_data-$cde_user"-telco" --local-path CDE_Demo/telco/data/ne_50m_admin_0_countries_lakes.zip -v
echo "Create CDE Resource for Job Dependencies"
cde resource create --name app_code-$cde_user"-telco" -v
...
cde job create --name geospatial_joins-$cde_user"-telco" --arg $data_lake --arg $cde_user --type spark --mount-1-prefix appCode/ --mount-1-resource app_code-$cde_user"-telco" --mount-2-prefix countriesData/ --mount-2-resource countries_data-$cde_user"-telco" --runtime-image-resource-name sedona-runtime-$cde_user"-telco" --packages org.apache.sedona:sedona-spark-shaded-3.0_2.12:1.5.0,org.datasyslab:geotools-wrapper:1.5.0-28.2 --application-file appCode/geospatial_joins.py -v
...
cde job create --name geo_orch-$cde_user"-telco" --type airflow --mount-1-prefix appCode/ --mount-1-resource app_code-$cde_user"-telco" --dag-file airflow.py -v
