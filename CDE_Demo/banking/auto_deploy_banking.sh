...
echo "Create CDE Docker Runtime dex-spark-runtime-great-expectations-data-quality-"$cde_user"-banking"
cde resource create --name dex-spark-runtime-great-expectations-data-quality-$cde_user"-banking" --image pauldefusco/dex-spark-runtime-3.2.3-7.2.15.8:1.20.0-b15-great-expectations-data-quality --image-engine spark3 --type custom-runtime-image
...
echo "Upload batch_load.py to cde_demo_files-"$cde_user
cde resource upload --name cde_demo_files-$cde_user"-banking" --local-path CDE_Demo/banking/spark/data_quality_orchestration.py

echo "Create job batch_load-"$cde_user"-banking"
cde job create --name batch_load-$cde_user"-banking" --type spark --mount-1-prefix jobCode/ --mount-1-resource cde_demo_files-$cde_user"-banking" --application-file batch_load.py --runtime-image-resource-name dex-spark-runtime-great-expectations-data-quality-$cde_user"-banking"
echo "Create job ge_data_quality-"$cde_user"-banking"
cde job create --name ge_data_quality-$cde_user"-banking" --type spark --mount-1-resource cde_demo_files-$cde_user"-banking" --application-file ge_data_quality.py --runtime-image-resource-name dex-spark-runtime-great-expectations-data-quality-$cde_user"-banking"
