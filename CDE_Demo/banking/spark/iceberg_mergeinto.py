...
#---------------------------------------------------
print("JOB STARTED...")
car_sales_df     = spark.sql("SELECT * FROM {0}.PERSONAL_LOANS_{1}".format(dbname, username)) #could also checkpoint here but need to set checkpoint dir
...
spark.sql("SELECT COUNT(*) FROM spark_catalog.{0}.PERSONAL_LOANS_{1}".format(dbname, username)).show()

ICEBERG_MERGE_INTO = "MERGE INTO spark_catalog.{0}.PERSONAL_LOANS_{1} t\
                      USING (SELECT * FROM spark_catalog.{0}.PERSONAL_LOANS_STAGING_{1}) s\
                      ON t.Loan_ID = s.Loan_ID\
                      WHEN MATCHED THEN UPDATE SET t.Loan_Status = s.Loan_Status\
