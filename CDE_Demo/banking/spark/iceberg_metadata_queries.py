...
try:
    spark.sql("SELECT * FROM {0}.PERSONAL_LOANS_{1}.history".format(dbname, username)).show()
...
QUERY = "WITH Ranked_Entries AS (\
    ...
    FROM \
        {0}.PERSONAL_LOANS_{1}.metadata_log_entries\
    WHERE \
...
QUERY = "SELECT * FROM {0}.PERSONAL_LOANS_{1}.metadata_log_entries;".format(dbname, username)
print(QUERY)
...
QUERY = "SELECT \
        ...
        FROM \
            {0}.PERSONAL_LOANS_{1}.snapshots;".format(dbname, username)
...
QUERY = "SELECT * FROM {0}.PERSONAL_LOANS_{1}.snapshots;".format(dbname, username)
