...
cde credential create --name dckr-crds-$cde_user"-mfct" --type docker-basic --docker-server hub.docker.com --docker-username $docker_user
echo "Create CDE Docker Runtime datagen-runtime-"$cde_user"-mfct"
cde resource create --name datagen-runtime-$cde_user"-mfct" --image pauldefusco/dex-spark-runtime-3.2.3-7.2.15.8:1.20.0-b15-dbldatagen-002 --image-engine spark3 --type custom-runtime-image

# CREATE FILE RESOURCE
echo "Create Resource files-"$cde_user"-mfct"
cde resource create --name files-$cde_user"-mfct"
echo "Upload utils.py to files-"$cde_user"-mfct"
cde resource upload --name files-$cde_user"-mfct" --local-path CDE_Resources/CDE_Files/utils.py
echo "Upload parameters.conf to files-"$cde_user"-mfct"
cde resource upload --name files-$cde_user"-mfct" --local-path CDE_Resources/CDE_Files/parameters.conf

# CREATE SETUP JOB
echo "Upload table_setup.py to files-"$cde_user"-mfct"
cde resource upload --name files-$cde_user"-mfct" --local-path CDE_Demo/manufacturing/spark/table_setup.py
echo "Create job table_setup-"$cde_user"-mfct"
cde job create --name table_setup-$cde_user"-mfct" --type spark --mount-1-resource files-$cde_user"-mfct" --application-file table_setup.py --runtime-image-resource-name datagen-runtime-$cde_user"-mfct"
...
cde resource delete --name files-$cde_user"-mfct"
...
cde resource upload --name files-$cde_user"-mfct" --local-path CDE_Demo/manufacturing/spark/staging_table.py
echo "Uploading iceberg_mergeinto.py to files-"$cde_user"-mfct"
cde resource upload --name files-$cde_user"-mfct" --local-path CDE_Demo/manufacturing/spark/iceberg_mergeinto.py
echo "Uploading airflow_DAG.py to files-"$cde_user"-mfct"
cde resource upload --name files-$cde_user"-mfct" --local-path CDE_Demo/manufacturing/airflow/airflow_DAG.py
echo "Uploading iceberg_metadata_queries.py to files-"$cde_user"-mfct"
cde resource upload --name files-$cde_user"-mfct" --local-path CDE_Demo/manufacturing/spark/iceberg_metadata_queries.py
echo "Creating Spark Job staging_table-"$cde_user"-mfct"
cde job create --name create_staging_table-$cde_user"-mfct" --type spark --mount-1-resource files-$cde_user"-mfct" --application-file staging_table.py --runtime-image-resource-name datagen-runtime-$cde_user"-mfct" --executor-cores 4 --executor-memory "4g"
echo "Creating Spark Job iceberg_mergeinto-"$cde_user"-mfct"
cde job create --name iceberg_mergeinto-$cde_user"-mfct" --type spark --mount-1-resource files-$cde_user"-mfct" --application-file iceberg_mergeinto.py --executor-cores 4 --executor-memory "4g"
echo "Creating Spark Job iceberg_metadata_queries-"$cde_user"-mfct"
cde job create --name iceberg_metadata_queries-$cde_user"-mfct" --type spark --mount-1-resource files-$cde_user"-mfct" --application-file iceberg_metadata_queries.py --executor-cores 4 --executor-memory "4g"
